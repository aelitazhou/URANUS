...
from env_map import MOESimulator
...
sim.ti = 24  # Starting from 12/1/2013 Monday 4.a.m. (a work day, and see the difference...)
...
for s in range(step_size):
    t = time.time()
    loc = sim.locs
    next_loc = np.argsort(np.mean(preds[sim.ti][:, 0:params['pred_grid'], 0], axis=1))[-params['num_agents']:]   # max dmd grid for agents
    action = []
    ...
    o, r, done, inf = sim.step(new_action)
...
np.save('RLAUP/NA_' + save_var + '_R', rewards)
np.save('RLAUP/NA_' + save_var + '_L', leftovers)
np.save('RLAUP/NA_' + save_var + '_A', actions)
np.save('RLAUP/NA_' + save_var + '_U', utilities)
np.save('RLAUP/NA_' + save_var + '_P', penalties)
