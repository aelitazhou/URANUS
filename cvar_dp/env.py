...
class MOESimulator(Env):
    ...
    def __init__(self, params, data, dmds, nbrs = None, sorted_grids = None, preds = None, gridmap = None):
    ...
    def solve_cvar_sdp(self, mu, std, z, grid_type = 0):
        ...
        for k in range(params['num_resources']):
            ...
            constraints = [ beta == (-params['f_consts'][grid_type][k] + params['r_consts'][k]) / params['g'][k] - nu[k][1, 1] + z ]
        #constraints += [ cp.trace(M @ Omega) / eps + beta <= 0 ]

        # Objective
        obj = cp.trace(M @ Omega) / eps + beta
    ...
    def solve_cvar_sdp(self, mu, std, z, grid_type = 0):
        ...
        return obj, (mu, std, z)  # Return the amount of excessive resources needed for each resource type; numerical errors addressed by explicitly judging for 0
        # return val
    ...
    def dp_stay(self, ti, t_s, locs, previous):
        ...
        # penalty
        leftover, feature = self.solve_cvar_sdp( mu, std, previous, grid_type)
        leftover_prev, feature_prev = self.solve_cvar_sdp( mu, std, (previous-1), grid_type)
        ...
        self.cvar_y = leftover
        self.cvar_x = feature
        self.cvar_y_p = leftover_prev
        self.cvar_x_p = feature_prev
        '''
        print('dp_stay--------------------------')
        print('dp time:', ti)
        print('loc:', locs)
