...
from env_drl_map import MOESimulator
...
# experiment
env = DummyVecEnv([lambda: MOESimulator(params, datax, dmds, grid_map, nbrs=None, sorted_grids=sorted_grids, preds=preds, gridmap=gridmap)])
model_fn = 'a2c_' + str(params['pred_grid']) + '_' +  str(params['num_agents'])
drl_model = A2C("MultiInputPolicy", env, verbose=0, n_steps=6, learning_rate=0.0001)
...
t = time.time()
# Train
model_path = '/mnt/data/fzhou/RP_RP_data/a2c/'
for itr in range(n_time):
    print('Iteration:', itr)
    tt = time.time()
    for day in range(n_day):
        drl_model.env.set_attr('start_ti', 144*order[itr][day])
        drl_model.env.set_attr('end_ti', 144*(order[itr][day]+1))
        drl_model.learn(total_timesteps=144, callback=SimCallback(fn=model_fn+'.log', append=True))
    # save every ten epoch and the last ten epoch
    if  itr >= 990:
        drl_model.save( model_path + model_fn + '_' + str(itr+1) )
    print('Time:', time.time()-tt)
print('Training time:', time.time()-t)
#drl_model.save( 'a2c/' + model_fn )
...
for itr in range(n_time):
    ...
    # load model
    if  itr >= 990:
        drl_model.load( model_path + model_fn + '_' + str(itr+1) )
    # test
    #if (itr+1) % 10 == 0 and itr < 990 or itr >= 990:
        leftovers = []
        rewards = []
        actions = []
        penalties = []
        ...
        sim = MOESimulator(params, datax, dmds, grid_map, nbrs=None, sorted_grids=sorted_grids, preds=preds, gridmap=gridmap)
        sim.start_ti = 24 + 144*15
        obs = sim.reset(preds = preds, gridmap = gridmap)
        step_size = len(preds) - sim.start_ti - 1
        t = time.time()
        ...
        save_var = str(params['pred_grid']) + '_' + str(params['num_agents']) + '_' + str(itr+1)

        np.save('RLAUP/A2C_' + save_var + '_R', rewards)
        np.save('RLAUP/A2C_' + save_var + '_L', leftovers)
        np.save('RLAUP/A2C_' + save_var + '_A', actions)
        np.save('RLAUP/A2C_' + save_var + '_U', utilities)
        np.save('RLAUP/A2C_' + save_var + '_P', penalties)
