...
class MOESimulator(Env):
    ...
    def __init__(self, params, data, dmds, grid_map, model, nbrs = None, sorted_grids = None, preds = None, gridmap = None):
        ...
        self.pred_grid = params['pred_grid']
    ...
    def reloc(self, action):
        action_new = action.copy()
        new_locs = self.locs.copy()
            ...
            if action[2*a] == 2:
                ...
                if action[2*a+1] == 100:  # in the moving process, action state = 2
                    ...
                    if self.grid_mp[a][sor][des] == 1:
                        ...
                        #print('renew')
                        #print(self.grid_map[sor][des])
                        #print(self.grid_mp[sor][des])
                        action_new[2*a+1] = des
                        new_locs[a] = des
        self.locs = new_locs
        '''
    ...
    def dp_stay(self, ti, t_s, locs, previous):
        ...
        # 4f
        #msz = torch.FloatTensor([mu, std, previous, self.pred_grid])
        #msz_prev = torch.FloatTensor([mu, std, previous-1, self.pred_grid])
        
        leftover = self.params['scalar_back'] * self.model(msz.cuda()).cpu().detach().numpy() 
        leftover_prev = self.params['scalar_back'] * self.model(msz_prev.cuda()).cpu().detach().numpy()  
