...
model = MLP().cuda()
model.load_state_dict(torch.load('learn_cvar/Lcvar.model'))
model.eval()

# experiment
sim = MOESimulator(params, datax, dmds, model, nbrs=None, sorted_grids=sorted_grids)
sim.nbrs = None
sim.reset(preds=preds, gridmap=gridmap)
sim.ti = 168 + 144*15  # Starting from the Monday (a work day, and see the difference...)
...
#save_var = str(params['penalty_w']) + '_' + str(params['pred_grid']) + '_' + str(params['num_agents']) + str(params['eps'])
save_var = str(params['pred_grid']) + '_' + str(params['num_agents'])

np.save('RLAUP/DP_' + save_var + '_R', rewards)
np.save('RLAUP/DP_' + save_var + '_L', leftovers)
np.save('RLAUP/DP_' + save_var + '_A', actions)
np.save('RLAUP/DP_' + save_var + '_U', utilities)
np.save('RLAUP/DP_' + save_var + '_P', penalties)
