...
from env_mdp_map import MOESimulator
                ...
                for j in range(n_g):
                    ...
                    for k in range(n_g):
                        if j == k:
                            ...
                            if pre_reward + score > temp:
                                temp = pre_reward + score
                                prev = k
                                action = 2
...
model = MLP().cuda()
model.load_state_dict(torch.load(params['model_cvar']))
model.eval()

# experiment
sim = MOESimulator(params, datax, dmds, grid_map, model, nbrs=None, sorted_grids=sorted_grids)
sim.nbrs = None
...
np.save('RLAUP/MDP_' + save_var + '_R', rewards)
np.save('RLAUP/MDP_' + save_var + '_L', leftovers)
np.save('RLAUP/MDP_' + save_var + '_A', actions)
np.save('RLAUP/MDP_' + save_var + '_U', utilities)
np.save('RLAUP/MDP_' + save_var + '_P', penalties)
