...
def dp_agent(sim, params):
    ...
    # dp process
    for dp_idx in range(len(dp_idxs)):
        rewards = []   # parallely compare multiple agents
        for a in dp_idxs: 
            ...
            if n_t != 1:
                for i in reversed(range(n_t-1)):    # i in [0,10]
                    for j in range(n_g):
                        ...
                        m_map[j][i] = prev
                        r_map[j][i] = temp
            rewards.append((temp, prev, action, m_map))
        max_temp, max_prev, max_action, max_m_map = max(rewards, key=lambda x: x[0])
        max_idx = rewards.index((max_temp, max_prev, max_action, max_m_map))
        #print(max_idx)
        #print(dp_idxs)

        step = max_prev
        agent_map[sim.locs[dp_idxs[max_idx]]][0] += 1
        agent_map[step][1] += 1
        for n in range(1, n_t-1):     # n in [1, 10]
            step = max_m_map[int(step)][n]
            agent_map[int(step)][n+1] += 1

        actions[2*a] = max_action
        actions[2*a+1] = max_prev
...
for s in range(step_size): 
    ...
    print('Time Step:', sim.ti, flush=True)
...
np.save('RLAUP/SDP_M_M_' + save_var + '_R', rewards)
np.save('RLAUP/SDP_M_M_' + save_var + '_L', leftovers)
np.save('RLAUP/SDP_M_M_' + save_var + '_A', actions)
np.save('RLAUP/SDP_M_M_' + save_var + '_U', utilities)
np.save('RLAUP/SDP_M_M_' + save_var + '_P', penalties)
print('files saved')
