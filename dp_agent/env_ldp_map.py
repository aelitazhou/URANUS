...
class MOESimulator(Env):
    ...
    def dp_stay(self, ti, t_s, locs, previous):
        ...
        # penalty
        if (np.array(ress - f) > 0).all():
            leftover = 0
        else:
            leftover = np.max( (f - ress) / self.params['f_coeffs'] ) 
        
        if (np.array(ress_prev - f) > 0).all():
            leftover_prev = 0
        else:
            leftover_prev = np.max( (f - ress_prev) / self.params['f_coeffs'] ) 
