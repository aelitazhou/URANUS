...
step = 12
tw_l = tw + step -1
train_seq = np.array(create_in_data(train[0], tw_l))
for i in range(train.shape[0]-1):
    train_seq = np.vstack((train_seq, create_in_data(train[i+1], tw_l)))
...
for i in range(epochs):
    ...
    for j, (seq, label) in enumerate(dataloader_train):
        ...
        # blstm
        loss = model.sample_elbo(inputs=seq, labels=label,
                                 criterion=criterion, sample_nbr=3,
                                 complexity_cost_weight=1/train_seq.shape[0])
        loss.backward()
        optimizer.step()
